...
class InvoiceController extends Controller
{
    ...
    return redirect()->route('user.invoice.all')->with($notification);
    } // End Method
    ...
    public function UserInvoiceAll(){
        $allData = Invoice::orderBy('date','desc')->orderBy('id','desc')->where('status','1')->get();
            return view('userbackend.invoice.invoice_all',compact('allData'));
    ...
    public function UserinvoiceAdd(){
        ...
        $date = date('Y-m-d');
        return view('userbackend.invoice.invoice_add',compact('invoice_no','category','date','costomer'));

    } // End Method


    public function UserInvoiceStore(Request $request){
    ...
    public function UserPendingList(){
        $allData = Invoice::orderBy('date','desc')->orderBy('id','desc')->where('status','0')->get();
            return view('userbackend.invoice.invoice_pending_list',compact('allData'));
    } // End Method



    public function UserInvoiceDelete($id){
    ...
    public function UserInvoiceApprove($id){

        $invoice = Invoice::with('invoice_details')->findOrFail($id);
        return view('userbackend.invoice.invoice_approve',compact('invoice'));

    }// End Method


    public function UserApprovalStore(Request $request, $id){
    ...
    public function UserPrintInvoiceList(){

    $allData = Invoice::orderBy('date','desc')->orderBy('id','desc')->where('status','1')->get();
       return view('userbackend.invoice.print_invoice_list',compact('allData'));
    } // End Method


    public function UserPrintInvoice($id){
        $invoice = Invoice::with('invoice_details')->findOrFail($id);
        return view('userbackend.pdf.invoice_pdf',compact('invoice'));

    } // End Method


    public function UserDailyInvoiceReport(){
        return view('userbackend.invoice.daily_invoice_report');
    } // End Method


    public function UserDailyInvoicePdf(Request $request){
        ...
        $start_date = date('Y-m-d',strtotime($request->start_date));
        $end_date = date('Y-m-d',strtotime($request->end_date));
        return view('userbackend.pdf.daily_invoice_report_pdf',compact('allData','start_date','end_date'));
    } // End Method
